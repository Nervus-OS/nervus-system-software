...
nspkg {
    ...
    // perm.system.launch：桌面点开别的应用、sessiond 唤起桌面，都要它。
    ...
    components.app("desktop") {
        ...
        runtime = "jvm"
        // manual：桌面不该被任何 Resolve 顺带拉起，只由 sessiond 显式启动。
    ...
    // 所以要一个 always-on 的 service 在开机后主动 Resolve 桌面的接口。
    //
    // criticality 用 optional 而不是 vital：vital 组件熔断（10 秒内崩 5 次）会触发
    // 整机 Safety 锁存让机器人停下来。桌面起不来是难看，不是危险。
    components.service("sessiond") {
        mainClass = "com.nervus.launcher.SessiondKt"
        runtime = "jvm"
